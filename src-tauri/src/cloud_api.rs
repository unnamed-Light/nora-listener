...
pub fn summarize_text(
    ...
    let context_section = if let Some(ctx) = context_text {
        if !ctx.trim().is_empty() {
            format!(
                "\n\nДОПОЛНИТЕЛЬНЫЕ МАТЕРИАЛЫ К ЛЕКЦИИ (КОНТЕКСТ ДЛЯ УМНОГО СИНТЕЗА):\n{}\n\n\
                ИНСТРУКЦИЯ ПО ИСПОЛЬЗОВАНИЮ ДОПОЛНИТЕЛЬНЫХ МАТЕРИАЛОВ:\n\
                1. Сверяй термины, имена ученых и формулы со слайдами/литературой для безупречной академической точности.\n\
                2. Если студент прикрепил свои заметки, адаптируй подачу материала под структуру и стиль мышления студента.\n\
                3. Если на слайдах или в методичке содержатся ключевые формулы, схемы или определения по теме, которые лектор пропустил устно, аккуратно дополни ими соответствующие подразделы с явной пометкой: [Из материалов лекции / слайдов].",
                ctx
            )
        } else {
            String::new()
    ...
    let base_prompt_text = if let Some(cp) = custom_prompt {
        if !cp.trim().is_empty() {
            cp.trim().to_string()
        } else {
            String::new()
